...
def create_jpk(
	...
	- system_name (string) - name of application creating JPK
	- file_name (string) - name for output file (with extension), or None if 
	...
	# (I'm not sure if it's important, but it doesn't require any effort)
...
def create_file(root, file_name):
	...
	# TODO: werify if file name is correct, doesn't exist and writing is possible
...
def get_is_guidance_accepted():
	...
	print("\t\t\t\t" + _("GUIDANCE") + "\n")
	print(_("This is experimental app, provided \"as is\", with NO WARRANTY!") + "\n")
...
def get_forwarded_excess_of_input_tax():
	...
	# amount should contain digits only
	if answer.isdecimal():
		return answer
	elif answer == "":
		return "0"
...
def create_jpk_header(purpose, tax_office_code, month, year, system_name, creation_date = None):
	...
	- purpose (string): "1" if it's first JPK for given period,
	  or "2" if it's amendment of already sent JPK
	- tax_office_code (string): 4-digit code of Tax Office that will
	  receive the JPK
...
def create_jpk_evidence(input_tax_documents, output_tax_documents, sum_of_field_k, sum_of_bad_debt_relief):
	...
	# IMPORTANT! Governmet server expects rows in below order:
	# - output tax rows
	# - output tax control
	# - input tax rows
	# - input tax control
	...
	for row in output_tax_rows:
		...
		# <tns:SprzedazCtrl> - output tax control row
...
def create_jpk_declaration(is_guidance_accepted, sum_of_field_k, sum_of_bad_debt_relief, forwarded_excess_of_input_tax = "0", amendment_reasons = ""):
	...
	# IMPORTANT! Government server does not accept xml if elements are in
	# different order. They have to be sorted ascending.
	for number, value in sorted(p_value.items()):
	...
	if amendment_reasons:
		p = create_p_element("ORDZU", amendment_reasons)
		if p is not None:
			declaration_details_element.append(p)
